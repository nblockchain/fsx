...
type AsyncExtensions() =
    ...
    member __.``basic test for Async.Choice``() =
        ...
        let shortSuccessfulJob =
            async {
                do!
                    Async.Sleep(
                        int shortTime.TotalMilliseconds
                        + int shortTime.TotalMilliseconds
                    )
    ...
    member __.``basic test for WhenAnyAndAll``() =
        ...
        let shortJob =
            async {
                lock
                    lockObj
                    (fun _ ->
                        asyncJobsPerformedCount <- asyncJobsPerformedCount + 1
                    )
        ...
        let stopWatch = Stopwatch.StartNew()
        subJobs |> Async.RunSynchronously |> ignore<array<int>>
        Assert.That(asyncJobsPerformedCount, Is.EqualTo 2)
    ...
    member __.``AsyncChoice cancels slower jobs (all jobs that were not the fastest)``
        ...
        let result =
            Async.Choice [ longJob; shortJob ] |> Async.RunSynchronously
    ...
    member __.``Async.MixedParallel3 cancels all jobs if there's an exception in one'``
        ...
        let result =
            try
                FSharpUtil.AsyncExtensions.MixedParallel3
                    longJob
                    shortJob
                    longJob2
                |> Async.RunSynchronously
    ...
    member __.``Async.MixedParallel4 cancels all jobs if there's an exception in one'``
        ...
        let result =
            try
                FSharpUtil.AsyncExtensions.MixedParallel4
                    longJob
                    shortJob
                    longJob2
                    longJob3
