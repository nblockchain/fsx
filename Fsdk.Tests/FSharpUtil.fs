...
type FSharpUtilCoverage() =
    ...
    member __.``find exception: basic test``() =
        ...
        let childFound =
            FSharpUtil.FindException<TaskCanceledException> wrapperEx
    ...
    member __.``find exception: it works with inherited classes (UnexpectedTaskCanceledException is child of TaskCanceledException)``
        ...
        let parentFound =
            FSharpUtil.FindException<TaskCanceledException> inheritedEx
    ...
    member __.``find exception: flattens (AggregateEx)``() =
        ...
        let sibling1Found =
            FSharpUtil.FindException<TaskCanceledException> parent

        match sibling1Found with
        | None -> failwith "should work"
        | Some ex -> Assert.That(Object.ReferenceEquals(ex, innerEx1), Is.True)

        let sibling2Found =
            FSharpUtil.FindException<UnexpectedTaskCanceledException> parent

        match sibling2Found with
        | None -> failwith "should find sibling 2 too"
        | Some ex -> Assert.That(Object.ReferenceEquals(ex, innerEx2), Is.True)
