...
module Taiga =
    ...
    let GetProjectIdBySlug authToken projectSlug : int =
        ...
        let response =
            webClient.DownloadString(
                sprintf "%s/projects/by_slug?slug=%s" TAIGA_API_URL projectSlug
            )

        let jsonResponseDict =
            JavaScriptSerializer()
                .Deserialize<Dictionary<string, obj>> response
        ...
        let projectId =
            ...
            | _ ->
                failwith(
                    "JSON response didn't include a string 'id' element? "
                    + response
                )

        projectId

    let NumberOfUserStoriesInIssueByProjectIdAndIssueId
        authToken
        (projectId: int)
        refId
        : Option<int> =
        ...
        let response =
            try
                Some(
                    webClient.DownloadString(
                        sprintf
                            "%s/issues/by_ref?ref=%d&project=%d"
                            TAIGA_API_URL
                            refId
                            projectId
                    )
        ...
        | Some likelyJsonResponse ->
            let jsonResponseDict =
                JavaScriptSerializer()
                    .Deserialize<Dictionary<string, obj>> likelyJsonResponse
            ...
            let generatedUserStories =
                ...
                | true, elementWithUnexpectedType ->
                    failwith(
                        sprintf
                            "Unexpected element of type %s"
                            (elementWithUnexpectedType.GetType().FullName)
                    )
    ...
    let GetAuthToken username password =
        ...
        let authToken =
            ...
            | _ ->
                failwith(
                    "JSON response didn't include a string 'auth_token' element? "
                    + response
                )
