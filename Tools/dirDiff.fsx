...
let compareFolders
    ...
    if count1 <> count2 then
        ...
        eprintfn
            "  Folder 1 has %d files (%d visible, %d hidden)"
            count1
            visible1
            hidden1

        eprintfn
            "  Folder 2 has %d files (%d visible, %d hidden)"
            count2
            visible2
            hidden2
    ...
    if not(Map.isEmpty sizeDifferences) then
        ...
        |> Map.iter(fun name size1 ->
            ...
            eprintfn
                "  %s: %d bytes (folder 1) vs %d bytes (folder 2)"
                name
                size1
                size2
        )
    ...
    let updateProgress() =
        ...
        let filled =
            int(float completedFiles / float totalFiles * float barWidth)
    ...
    let filesWithDiffs =
        if isParallel then
            ...
            |> Array.iter(fun batch ->
                ...
                let batchResults =
                    asyncWorkItems |> Async.Parallel |> Async.RunSynchronously
...
let main(args: string []) : int =
    ...
    | None ->
        ...
        eprintfn
            "Compares two folders and checks if their contents are identical."
        ...
        eprintfn
            "  --non-parallel  Disable parallel processing (default: parallel)"
