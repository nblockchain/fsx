...
let BuildFsxScript(script: FileInfo) : BuildResult =
    ...
    let GetBinFolderForAScript(orig: FileInfo) =
    ...
    let rec preprocessScriptContents(origScript: FileInfo): seq<FileInfo> =
        let autogeneratedFile,_ = GetEquivalentFileForAutogeneratedCounterPartInBinSubfolder(origScript, "fs")
...
| Success(exeTarget) ->
    ...
    if not (parsedArgs.Flags.Contains(Flag.OnlyCompile)) then
        let runResult = Process.Execute(sprintf "mono %s" exeTarget.Exe.FullName, false, false)
