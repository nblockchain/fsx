...
let BuildFsxScript(script: FileInfo) (verbose: bool): BuildResult =
    if (script = null) then
    ...
    let preprocessScriptContents(origScript: FileInfo): List<CompilerInput> =
        ...
        let lineActions = readScriptContents origScript
        ...
        seq {
            ...
            for line,maybeDep in lineActions do
                match maybeDep with
                | LineAction.Normal ->
...
let GetAlreadyBuiltExecutable (exeTarget: FileInfo)
    ...
    if not (binFolder.Exists) then
        None
    else if (binFolder.LastWriteTime < lastWriteTimeOfSourceFile) then
        None
    else if not (exeTarget.Exists) then
        None
    else if (exeTarget.LastWriteTime < lastWriteTimeOfSourceFile) then
        None
    else
        Some(exeTarget)

let Build(generateArtifacts: bool) (verbose: bool) =
    let buildResult = BuildFsxScript parsedArgs.Script verbose
...
if (check || force) then
    ...
    Build generateArtifacts verbose
        |> ignore
    Environment.Exit 0

// FIXME: check dependencies of file too
let lastWriteTimeOfSourceFile = parsedArgs.Script.LastWriteTime

let binFolder,exeTarget = GetAutoGenerationTargets parsedArgs.Script "exe"
let maybeExe = GetAlreadyBuiltExecutable exeTarget binFolder lastWriteTimeOfSourceFile
